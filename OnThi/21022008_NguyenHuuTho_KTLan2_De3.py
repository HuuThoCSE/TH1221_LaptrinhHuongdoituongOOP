...
class NGUOIHOC():
    ...
    def Nhap(self):
        ...
        # Xu ly ngoai le ngay sinh
        checkdate = True
        while checkdate:
            try:
                nam = date(nam, thang, ngay)
                checkdate = False
            except(ValueError):
                checkdate = True
...
class NGUOIHOC_NC(NGUOIHOC):
    trinhdo = "CB"
    def __init__(self, ms="", ht="", ns=date(2022, 12, 9), dt=0.0, sdt=""):
        super(NGUOIHOC_NC, self).__init__(ms, ht, ns, dt)
        self.sdt = sdt
...
Nguoi3 = NGUOIHOC_NC("21022018", "Nguyen Huu Phuoc", date(2003, 8, 1), 10, "0355865681")
Nguoi3.Xuat()
print()
...
# Số học sinh sinh vào các ngày 5, 15, 25
dem = 0
for i in range(0, len(DSNH)):
    if (DSNH[i].ns.day == 5 or DSNH[i].ns.day == 15 or DSNH[i].ns.day==25):
        dem += 1
print(f"Có {dem} sinh viên sinh vào ngày 5, 15, 25.")
print()

# Tên của người học có số điện thoại bắt đầu từ 0919
print("Tên của người học có số diện thoại bắt đầu từ 0907: ")
for i in range(0, len(DSNH)):
    if (str(DSNH[i].sdt[:4]) == "0907"):
        print(DSNH[i].Ten())
print()
